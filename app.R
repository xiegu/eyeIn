...
load('data/eyeIn_data.RData')
# Simulation data
load('data/finScoreDemo.RData')

# warning! finscoredemo update using sales data from sales 1218
# finScoreDemo <-
#   mutate(
#     finScoreDemo,
...
server <- function(input, output) {
    ...
    if (user_input$authenticated == FALSE) {
      ...
      tabItems(
        tabItem(
          ...
          fluidRow(column(
            2,
            selectInput('mapCity', '城市', choices = cityList)
          )),
          br(),
          fluidRow(
            tags$style("
        ...
        tabItem(
          ...
          fluidRow(
            column(2,
                   selectInput('scoreCity2', '城市', choices = cityList)),
            column(2,
  ...
  output$mapAll <- renderLeaflet({
    cityMap(input$mapCity)
    
  })
  
  output$haierTable <- renderDataTable({
    tmpTable <- filter(haierTable, city == input$mapCity) %>%
      select(.,
             province,
             city,
             district,
  ...
  output$GreeTable <- renderDataTable({
    tmpTable <- filter(gree, city == input$mapCity) %>%
      select(.,
             province,
             city,
             district,
  ...
  output$MideaTable <- renderDataTable({
    tmpTable <- filter(midea, city == input$mapCity) %>%
      select(.,
             province,
             city,
             district,
  ...
  output$scoreDistrict <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity)%>%pull(district) %>% unique
    
    selectInput('scoreDistrict', '行政区', choices = choiceList)
  })
  
  output$scoreShop <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict) %>% pull(name) %>% unique
  ...
  output$scoreDistrict2 <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity2)%>%pull(district) %>% unique
    
    selectInput('scoreDistrict2', '行政区', choices = choiceList)
  })
  
  output$scoreShop2 <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity2 & district == input$scoreDistrict2) %>% pull(name) %>% unique
  ...
  output$scoreDistrict3 <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity3)%>%pull(district) %>% unique
  ...
  output$scoreShop3 <- renderUI({
    choiceList <- filter(haierTable, city == input$scoreCity3 & district == input$scoreDistrict3) %>% pull(name) %>% unique
  ...
  output$scoreRadar <- renderHighchart({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    
    tmpTable2 <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict) %>%
      select(starts_with('score.'))
    
    tmpTable3 <- filter(haierTable, city == input$scoreCity) %>%
      select(starts_with('score.'))
  ...
  output$networkScore <- renderDataTable({
    tmpTable <- filter(haierTable, city == input$scoreCity) %>%
  ...
  output$place <- renderValueBox({
    ...
    valueBox(
      subtitle = paste(input$scoreCity, '信息中心指数', sep = ' '),
  ...
  output$life <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    valueBox(
      subtitle = paste(input$scoreCity, '生活中心指数', sep = ' '),
  ...
  output$traffic <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    valueBox(
      subtitle = paste(input$scoreCity, '交通中心指数', sep = ' '),
  ...
  output$business <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    valueBox(
      subtitle = paste(input$scoreCity, '商业中心指数', sep = ' '),
  ...
  output$development <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    valueBox(
      subtitle = paste(input$scoreCity, '发展中心指数', sep = ' '),
  ...
  output$competition <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name %in% input$scoreShop) %>%
      select(starts_with('score.'))
    valueBox(
      subtitle = paste(input$scoreCity, '竞争中心指数', sep = ' '),
  ...
  output$cityGDP <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity) %>% 
      select(., starts_with('city')) %>% unique
  ...
  output$mapPlace <- renderLeaflet({
    placeMap(chengshi = input$scoreCity, shop = input$scoreShop)
  })
  ...
  output$cityDensity <- renderHighchart({
    tmpTable <- filter(china_pop_area, city == input$scoreCity & division != input$scoreCity)
  ...
  output$scoreBestWorst <- renderHighchart({
    tmpTable <- filter(haierTable, city == input$scoreCity) %>% select(., city, shopCode, name, score.final) %>% unique
  ...
  output$cityDev1 <- renderHighchart({
    GDPTable <- filter(GDP_topCity_China, city == input$scoreCity)
    PopTable <- filter(Pop_topCity_China, city == input$scoreCity)
    highchart() %>%
      hc_chart(type = 'line') %>%
      hc_yAxis_multiples(
        list(
      ...
      hc_tooltip(pointFormat = "<span style=\"color:{series.color}\">{series.name}</span>:
                       {point.y:,.2f}<br/>",
                 shared = TRUE) %>%
      hc_xAxis(categories = rev(colnames(GDPTable)[-c(1,7,8)]),
               labels = list(style = list(color = 'white'))) %>%
      hc_add_series(
        data = as.numeric(select(GDPTable, rev(
          colnames(GDPTable)[-c(1,7,8)]
        ))),
        color = 'LightCoral',
        name = 'GDP',
        yAxis = 1
  ...
  output$cityDev2 <- renderHighchart({
    SalaryTable <- filter(Salary_topCity_China, city == input$scoreCity)
    PropertyPriceTable <- filter(PropertyPrice_topCity_China, city == input$scoreCity)
    highchart() %>%
      hc_yAxis_multiples(
        list(
          lineWidth = 1,
      ...
      hc_tooltip(pointFormat = "<span style=\"color:{series.color}\">{series.name}</span>:
                       {point.y:,.0f}<br/>",
                 shared = TRUE) %>%
      hc_xAxis(categories = rev(colnames(SalaryTable)[-c(1,7,8)]),
               labels = list(style = list(color = 'white'))) %>%
      hc_add_series(
        data = as.numeric(select(SalaryTable, rev(
          colnames(SalaryTable)[-c(1,7,8)]
        ))),
        color = 'LightCoral',
        name = '平均工资',
        yAxis = 1
      ...
      hc_add_series(data = as.numeric(select(PropertyPriceTable, rev(
        colnames(PropertyPriceTable)[-c(1,7,8)]
      ))),
      color = 'SteelBlue',
      name = '住宅均价') %>%
      hc_legend(itemStyle = list(color = 'white'))
  ...
  #     tmpTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name == input$scoreShop) %>%
  ...
  #     placeTable <- filter(haierTable, city == input$scoreCity & district == input$scoreDistrict & name == input$scoreShop)
  #   
  #   placeName <-
  #     c(
  #       'resid_500',
  ...
  output$revCompChart <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_yAxis(title = list(text = '收入', style = list(color = 'white')),
               labels = list(style = list(color = 'white'))) %>%
      hc_add_series(
        data = filter(table1, name == '河南清河工贸CBD店')$revenue,
        type = 'column',
        color = 'rgba(64,224,208,0.8)',
        name = '河南清河工贸CBD店-收入'
      ) %>%
      hc_add_series(
        data = filter(table1, name == '郑州锐达CBD店')$revenue,
        type = 'column',
        color = 'rgba(240,128,128,0.8)',
        name = '郑州锐达CBD店-收入'
      ) %>%
      hc_add_series(
        data = filter(table1, name == '郑州丹尼斯人民路锐达店')$revenue,
        type = 'column',
  ...
  output$trafficCompChart <- renderHighchart({
    ...
    table1 <- finScoreDemo %>%
      mutate(
        ym_quantity = round(ifelse(is.na(revenue), 1011, revenue/100 + round(runif(1)*200)))
      )
  ...
  output$lyRevenue <- renderValueBox({
    tmpTable <- filter(haierTable, city == input$scoreCity2 & district == input$scoreDistrict2 & name == input$scoreShop2)
    col <- 'black'
    valueBox(
